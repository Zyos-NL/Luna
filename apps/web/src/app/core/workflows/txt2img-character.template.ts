...
/**
 * Mirror of `infra/workflows/txt2img-character.json` (21 nodes, with PuLID).
 * Daily-driver: Flux + 2 LoRAs + PuLID-Flux (weight 0.8, end_at 0.7) +
 * FaceDetailer. ApplyPulidFlux node #24 wraps the model chain that the
 * KSampler and FaceDetailer consume.
 *
 * Maintained by the character-pipeline agent. Read-only — WorkflowService
 * deep-clones before mutating.
 */
export const TXT2IMG_CHARACTER_TEMPLATE: BuiltWorkflow = {
  '1': {
  ...
  '24': {
    ...
    _meta: { title: 'Apply PuLID-Flux (weight=0.8, end=0.7 — daily driver)' },
  },
  '6': {
  ...
  '16': {
    ...
    inputs: {
      images: ['15', 0],
      filename_prefix: 'characters/test/scene',
    },
    _meta: { title: 'Save Image -> outputs/characters/<id>/scene_*' },
  },
};
