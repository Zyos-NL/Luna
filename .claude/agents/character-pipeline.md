...
- **PuLID-Flux node = `lldacing/ComfyUI_PuLID_Flux_ll`** (NIET balazik/...-Enhanced, NIET sipie800/...-Enhanced). Workflow JSON gebruikt class names: `ApplyPulidFlux`, `PulidFluxModelLoader`, `PulidFluxInsightFaceLoader` (input `provider` enum CPU/CUDA/ROCM — gebruik `CUDA`), `PulidFluxEvaClipLoader` (geen inputs, auto-resolve), `PulidFluxFaceDetector`. Wire `ApplyPulidFlux` ná de LoRA-chain en vóór KSampler.
- **`FixPulidFluxPatch` is momenteel uitgeschakeld**: incompatibel met ComfyUI 0.19.3 (lldacing's `pulid_forward_orig` signature mist `**kwargs` en crasht op het nieuwe `timestep_zero_index=` kwarg). `infra/install-nodes.sh` patcht dit runtime via een Python-blok dat `**kwargs,` toevoegt aan de signature. Workflows omitten de Fix-node zelf tot lldacing upstream fixt. Single-shot workflows zijn veilig zonder, alleen multi-PuLID-disconnect-cycles binnen één workflow zouden model-pollution riskeren — niet onze use-case.
